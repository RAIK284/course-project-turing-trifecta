...
const OpposingGhostGuess: React.FC<GamePageProps> = ({ game, round, user }) => {
  const { gameSessionStore } = useStore();
  const { opposingGhostGuess, callingEndpoint } = gameSessionStore;
  const [isLeft, setLeft] = useState<boolean | undefined>(
    round.opposingGhostGuesses?.find((gg) => gg.userId === user.id)?.isLeft
  );

  const handleSendGuess = () => {
    opposingGhostGuess(game.id, !!isLeft);
  };
      ...
      {directions.canDoAction && (
        <>
          <div className="flex gap-3">
            <button
              className={cn(
                "uppercase rounded bg-theme-blue w-24 px-5 py-1 font-semibold",
...
export default OpposingGhostGuess;
